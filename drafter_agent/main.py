...
async def ocr_court_order_endpoint(file: UploadFile = File(...)):
    """Uploads an image of a court order and returns the extracted OCR text."""
    try:
        content_type = file.content_type
        if not content_type.startswith("image/"):
            raise HTTPException(status_code=400, detail="Only image files are supported currently for OCR")
            
        file_bytes = await file.read()
        logger.info(f"Extracting OCR text from uploaded {content_type} file")
